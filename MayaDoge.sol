...
contract MayaDoge is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the genesis pool & airdrop
    uint256 public constant INITIAL_MAYA_GENESIS_POOL = 24000 ether;
    uint256 public constant INITIAL_MAYA_AIRDROP = 1000 ether;
    ...
    /**
     * @notice Constructs the MAYA ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("MayaDoge", "MAYA") {
        // Mints 1 MAYA to contract creator for initial pool setup
    ...
    function _getMayaDogePrice() internal view returns (uint256 _mayadogePrice) {
        try IOracle(mayadogeOracle).consult(address(this), 1e18) returns (
            uint144 _price
        ) {
            return uint256(_price);
        } catch {
            revert("MayaDoge: failed to fetch MAYA price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _MayaDogePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_MayaDogePrice >= taxTiersTwaps[tierId]) {
    ...
    function setMayaDogeOracle(address _mayadogeOracle) public onlyOperatorOrTaxOffice {
        require(_mayadogeOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints MAYA to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of MAYA to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentMayaDogePrice = _getMayaDogePrice();
            currentTaxRate = _updateTaxRate(currentMayaDogePrice);
            if (currentMayaDogePrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        address _mayadogeGenesis,
        address _airdropReceiver
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_mayadogeGenesis != address(0), "!_mayadogeGenesis");
        require(_airdropReceiver != address(0), "!_airdropReceiver");
        rewardPoolDistributed = true;
        _mint(_mayadogeGenesis, INITIAL_MAYA_GENESIS_POOL);
        _mint(_airdropReceiver, INITIAL_MAYA_AIRDROP);
    }
